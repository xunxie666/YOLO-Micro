...
def layer_pruning(weights):
    ...
    """
    included_layers = []
    for layer in model.model[:100]:  # 获取backbone
    ...
    for m in model.modules():
        ...
        if isinstance(m, nn.Conv2d) and m in included_layers:
            # amount是剪枝率
            # 卷积剪枝
            pruning_plan = DG.get_pruning_plan(m, tp.prune_conv, idxs=strategy(m.weight, amount=0.7))
            logger.info(pruning_plan)
            # 执行剪枝
            pruning_plan.exec()
        if isinstance(m, nn.BatchNorm2d) and m in included_layers:
            # BN层剪枝
            pruning_plan = DG.get_pruning_plan(m, tp.prune_batchnorm, idxs=strategy(m.weight, amount=0.7))
            logger.info(pruning_plan)
            pruning_plan.exec()
    # 获得剪枝以后的参数量
