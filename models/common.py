...
class SAM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = self.conv1(x)
        return self.sigmoid(x).expand_as(inputs) * inputs
...
class ESAM(nn.Module):
    def __init__(self, channel, kernel_size=7, b=1, gama=2):
        super(ESAM, self).__init__()
        c = channel
        self.eca = ECA(c)
        self.sam = SAM(kernel_size)
        self.cv1 = Conv(2 * c, channel, 1, 1)  # 7,3     3,1


    def forward(self, inputs):
        x1 = self.eca(inputs)
        x2 = self.sam(inputs)
        out = self.cv1(torch.cat([x1, x2], dim=1))
        return out
...
class GSConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1):
        super().__init__()
        c_ = c2 // 2
        self.cv1 = Conv(c1, c_, k, s, None, g, act=nn.LeakyReLU(0.1))
        self.cv2 = Conv(c_, c_, 5, 1, None, c_, act=nn.LeakyReLU(0.1))
    ...
    def forward(self, x):
        ...
        b_n = torch.div(b * n, 2, rounding_mode='trunc')
        y = x2.reshape(b_n, 2, h * w)
        y = y.permute(1, 0, 2)
        y = y.reshape(2, -1, torch.div(n, 2, rounding_mode='trunc'), h, w)
...
class SPPCSPC_GS(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, e=0.5, k=(5, 9, 13)):
        super(SPPCSPC_GS, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = GSConv(c1, c_, 1, 1)
        self.cv2 = GSConv(c1, c_, 1, 1)
        self.cv3 = GSConv(c_, c_, 3, 1)
        self.cv4 = GSConv(c_, c_, 1, 1)
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
        self.cv5 = GSConv(4 * c_, c_, 1, 1)
        self.cv6 = GSConv(c_, c_, 3, 1)
        self.cv7 = GSConv(2 * c_, c2, 1, 1)
...
class G_ELAN(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.cv1 = Conv(c1, c1, 1, 1, act=nn.LeakyReLU(0.1))
        self.cv2 = GhostConv(c1, c1, 3, 1, act=nn.LeakyReLU(0.1))
        self.cv3 = Conv(4*c1, c2, 1, 1, act=nn.LeakyReLU(0.1))
    ...
    def forward(self, x):
        ...
        x3 = self.cv2(x2)
        x4 = self.cv2(x3)
        out = self.cv3(torch.cat([x1, x2, x3, x4], dim=1))
        return out
...
class SGBottleneck(nn.Module):
    def __init__(self, c1, c2, s, use_ca=False):
        super().__init__()
        c_ = c2 // 2
        self.cv1 = Conv(c1, c_, 1, 1, None, 1)
        self.cv2 = Conv(c_, c_, 1, 1, None, 1)
        self.cv3 = Conv(c1, c_, 3, 2, None, 1)
